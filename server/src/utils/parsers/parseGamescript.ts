...
import { createGSDataPacket } from "../packetCreators/createPackets";
        ...
        if ('data' in json) {
            ...
            switch (datatype) {
                case GameScriptDataType.CARGO:
                    activeConnection.emitNotification('message', { title: 'Logged cargoes', message: 'Finished parsing cargo data from GameScript.', context: MessageNotificationContext.SUCCESS })
                    break;
                case GameScriptDataType.INDUSTRY:
                    activeConnection.emitNotification('message', { title: 'Logged industries', message: 'Finished parsing industry data from GameScript.', context: MessageNotificationContext.SUCCESS })
                    break;
                case GameScriptDataType.INDUSTRY_TYPE:
                    activeConnection.emitNotification('message', { title: 'Logged industry types', message: 'Finished parsing industry types data from GameScript.', context: MessageNotificationContext.SUCCESS })
                    break;
                case GameScriptDataType.TOWN:
                    activeConnection.emitNotification('message', { title: 'Logged towns', message: 'Finished parsing town data from GameScript.', context: MessageNotificationContext.SUCCESS })
                    break;
                case GameScriptDataType.STATION:
                    activeConnection.emitNotification('message', { title: 'Logged stations', message: 'Finished parsing station data from GameScript.', context: MessageNotificationContext.SUCCESS})
                    break;
                case GameScriptDataType.MONTHLY_STATS:
                    activeConnection.emitNotification('message', { title: 'Logged monthly stats', message: 'Finished parsing monthly stats data from GameScript.', context: MessageNotificationContext.SUCCESS })
                    break;
                case GameScriptDataType.CARGO_WAITING_STATION_AT_VIA_FROM:
                    activeConnection.emitNotification('message', { title: 'Logged waiting cargoes', message: 'Finished parsing waiting cargoes data from GameScript.', context: MessageNotificationContext.SUCCESS })
                    break;
            case GameScriptDataType.COMPANY:
                    activeConnection.emitNotification('message', { title: 'Logged companies', message: 'Finished parsing companies data from GameScript.', context: MessageNotificationContext.SUCCESS})
                    break;
                default:
                    activeConnection.emitNotification('message', { title: 'Logged GS data', message: 'Finished parsing GS data from GameScript. Type unknown.', context: MessageNotificationContext.WARNING })
            }
            ...
            if (typeof nextRequest !== 'undefined') {
                ...
                activeConnection.emitNotification('message', { title: `Sending request to GameScript for type ${friendlyText}`, message: `Sending update request for update type: ${GSDataTypeFriendlyNames[nextRequest]}`, context: MessageNotificationContext.INFO });
                try {
